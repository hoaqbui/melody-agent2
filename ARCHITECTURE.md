...
- **the sidecar is the only remote shell, and Tailscale is its only gate** — it binds the tailnet interface, holds the `goose serve` token, and is never exposed by `tailscale funnel`; contract: review rule plus a startup refusal when the bind address is public.
