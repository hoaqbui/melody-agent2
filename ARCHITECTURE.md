...
- **ui/sidecar (new)** — the one process that touches the machine for the panes: pty (`node-pty`, sessions that outlive a client), fs read/write + `chokidar` watch, git commands, the `goose serve` WebSocket proxy that injects `?token=` (the secret never leaves the Mac), and the static web build. Reachable from the Electron renderer and from a browser on the tailnet alike; bound to the tailnet interface, never behind `tailscale funnel`. Amended 2026-09-16 (tasks 48, 54, 61): every `/git/*` body takes `cwd`, answered `400` unless its realpath is inside the spawn cwd's toplevel or a `.worktrees/` sibling of it — no route runs git elsewhere; worktrees exist only at `<toplevel>/.worktrees/<slug>` on `wt/<slug>` (`/git/worktree/{add,list,remove}`, `/git/merge` with `--no-ff`, `409` and the conflict list on a clash) and `.worktrees/` is invisible to the watcher; `/git/apply` is `git apply --recount` at the toplevel with the patch on stdin, all-or-nothing, the pane's only write to a hunk; `scheduler.rs` is the one other writer of `.worktrees/`, on the same slug shape, so Merge and Remove read a scheduled run's worktree like any other — a third writer is a bug; every machine route answers `401` without the per-launch key (§Invariants). Amended 2026-09-19 (tasks 73, 78, 80, 84, 94): `/fs/*` contained like `/git/*` — `requestPath(spawnCwd, body)` (`fs.ts`) resolves the realpath through the nearest existing ancestor and answers `400` unless it lies inside the spawn cwd's toplevel or a `.worktrees/` sibling; `/git/snapshot {cwd}` writes `add -A` into a temp index (`GIT_INDEX_FILE`) and returns the tree, the real index untouched, refused outside the toplevel; `/git/discard {cwd, path?}` stashes the working tree (or one path) and `/git/discard-undo` pops it, refused when the stash is not the one it made; `/runtimes/probe` runs four fixed argv arrays (`claude auth status --json`, `codex login status`, `cursor-agent status`, `agy models`) with a 5 s timeout each and never a shell, `ENOENT` or exit 127 reading as not installed. Amended 2026-09-20 (task 126): `/ledger/append {cwd, event}` and `/ledger/read {cwd, since?}` — the work ledger, one append-only JSONL per repository under goose's state dir (`<state>/goose/ledger/<basename>-<hash>.jsonl`), never inside the repo; `cwd` contained like `/fs/*` (the spawn cwd's toplevel, or the spawn cwd itself outside any repository, or a `.worktrees/` sibling); a worktree shares its repository's file; `kind` is one of `turn · worker · correction · confirm · review · undo · handoff`, else `400`. Amended 2026-09-23 (task 264): `kind` gains `land · verdict · link · gap` (a job's landing commit, the user's verdict, a rework link, and a closed-app gap); the `worker` event gains job identity — `turnId`, `member`, `charterSha`, `taskRef`, `taskHash`, `baseSha` — filled from the transcript where the renderer's builder can (`ledger-events.ts`); `charterSha` stays `null` until M2's `~/Melody/<name>/charter.md`, `baseSha` until a run-start capture exists, and `taskHash` until a builder reads `tasks.md` itself. Amended 2026-09-24 (task 271): a second, read-only root — the notebook (`MELODY_NOTEBOOK`, default `~/Melody`, canonicalized at spawn through its nearest existing ancestor even when the notebook itself does not exist yet, so containment still lines up once it's created) — served by `/notebook/list {dir}`, `/notebook/read {path, rev?}` (`rev` a 40-hex sha, run as `git show <sha>:./<path>`) and `/notebook/log {path?, since?}` (always re-validates the root and constrains history to it with a `./`-prefixed pathspec, never a bare `git log` of the whole repository `root` may sit inside), each refused `400` outside the root — contained like `/fs/*`'s `requestPath` (realpath through the nearest existing ancestor) but against the one fixed root, no git toplevel to fall back on, and a missing notebook reads `404`; no route writes it; git runs only `log` and `show`, never a shell. `/ledger/read {cwd, since?}` gains `name` as an alternative body key to `cwd` (exactly one when `name` is given, both `400`; a bare `*.jsonl` basename, a `/` or `..` refused, and the resolved file itself realpath-contained to the ledger dir so a symlink placed there can't read outward); `/ledger/list` names every `*.jsonl` in the ledger dir; both read-only, no new ledger kinds. `fileFor` now keys a ledger file on the request `cwd`'s own repository rather than always the spawn root's — a linked worktree (the one shape whose `--git-dir` differs from its `--git-common-dir`) folds back to its main repository wherever it lives, not only a `.worktrees/` sibling; a submodule or a `--separate-git-dir` repository has git-dir equal to common-dir and so keys on its own toplevel instead, keeping two submodules of one superproject distinct — containment is still the spawn cwd's toplevel or that `.worktrees/` sibling; only which file a cwd inside it writes to moved. `/fs/*` and `/git/*` unchanged.
...
- **components** — upstream UI: chat, tool-call rendering, permissions, settings, sessions list; kept, not rewritten. Amended 2026-09-22 (user: "consider this a full ui/ux fork of goose" → "Crates only"): `ui/desktop` is Melody's own — upstream merges bring `crates/*` only, and an upstream UI change lands by hand when the user asks for it; "it's upstream's" is no longer a reason to keep a UI name, drawing or component.
...
- **goose-cli / goose / goose-agent / goose-providers / goose-mcp** — the spine: sessions, modes, permissions, providers, `delegate`, star topology, handoff memo. The fork adds or edits three feature files inside `goose`, plus the provider registration points (`providers/mod.rs`, `providers/init.rs`, `inventory/registrations.rs`):
  ...
  - **Melody surface (new, task 207)** — Melody's own tools on the session bridge: `list_sessions`, `start_session(repo, task, mode, provider, model, extensions)`, `session_status`, in `src/acp/server/melody_surface.rs`. Unlike summon's tools, dispatched into the calling session's own `Agent`, these read and write the server's shared `SessionManager`/`AgentManager` (`SharedAcpState`, `acp/server.rs`'s `SharedAcpState`, built by `acp/server_factory.rs`'s `shared()`) directly, so creating and inspecting a session needs no client connection — the manager sessions `start_session` creates may never have a window open on them (208 gives one its first turn). `agents/session_bridge.rs` gained a `SessionTools` trait (`Registered.tools`, `SessionBridge::attach_tools`) so `agents/` need not depend on `acp/`; `acp/server.rs`'s `sync_session_bridge` attaches a `MelodySurface` to every activated session. A `start_session` missing `mode`, `provider`/`model` or `extensions` is refused — never a silent default (mirrors `new_session.rs`'s own rule against defaulting to `Config::global().get_goose_mode()`). Started sessions are `User`, role `manager`, parent Melody, built on 206's `get_or_create_manager`/`set_role`. `session/new` (`new_session.rs`) gained `_meta.role`, applied through `set_role` after the session's initial config, for M1b to mark a session as Melody's own or as a repository's manager from the ordinary session-creation path.
...
- **delegated work never nests** — a `SubAgent` session has no `delegate` tool and is refused if it calls it; contract: upstream `summon.rs:1622-1624`, `:2609-2621`, not re-implemented.
