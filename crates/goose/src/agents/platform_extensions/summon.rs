...
impl SummonClient {
    ...
    async fn handle_delegate(
        ...
        let (result, subagent_session_id) = loop {
            ...
            // One row per attempt: a quota-dead child ends `failed`, the
            // replacement gets its own `running`.
            ...
            let quota = match result {
                ...
                result => {
                    ...
                    yield_to_outer_tool_stream().await;
                    break (result, subagent_session_id);
                }
            };
            sink.lock().await.route(delegation_notification(
                DELEGATE_FINISHED_EVENT,
                &delegation_finished(delegation, &Err(anyhow::anyhow!("{quota}"))),
            ));
...
You research."#,
    ...
    async fn delegate_rerolls_on_quota_error() {
        ...
        for state_machine in [None, Some("1")] {
            ...
            let (notification_tx, mut notification_rx) = tokio::sync::mpsc::channel(32);
            let result = client
                .handle_delegate(
                    &parent.id,
                    arguments.as_object().cloned(),
                    CancellationToken::new(),
                    Some(ToolCallNotificationEmitter::new(notification_tx)),
                    None,
                )
                .await
                .unwrap();
            ...
            while let Ok(notification) = notification_rx.try_recv() {
                delegations.extend(delegation_update_from_notification(&notification));
            }
            let attempts: Vec<(&str, DelegationStatus)> = delegations
                .iter()
                .map(|update| (update.provider.as_str(), update.status))
                .collect();
            assert_eq!(
                attempts,
                [
                    (QUOTA_STUB, DelegationStatus::Running),
                    (QUOTA_STUB, DelegationStatus::Failed),
                    (ANSWER_STUB, DelegationStatus::Running),
                    (ANSWER_STUB, DelegationStatus::Done),
                ],
                "state_machine={state_machine:?}: {delegations:?}"
            );
            assert_eq!(
                delegations[0].subagent_session_id,
                delegations[1].subagent_session_id
            );
            assert_ne!(
                delegations[1].subagent_session_id,
                delegations[2].subagent_session_id
            );
            assert!(delegations[1].error.is_some());
            assert!(delegations[3].error.is_none());
            ...
            assert_eq!(record.from.session_id, delegations[1].subagent_session_id);
            assert_eq!(child_id, delegations[3].subagent_session_id);
    ...
    async fn test_resolve_provider_recipe_overrides_env_var() {
        ...
        let (resolved_provider, _) = client
            .resolve_provider(
                ...
                &session_with(parent_config()),
                &[],
                &session_with(parent_config()).working_dir,
            )
            .await
