...
- **Order.** Wave 1 (disjoint files): 73 fs guard ∥ 74 one poll ∥ 75
  ...
  Wave 2: 82 `file:line` links (75) ∥ 83 Add to chat ×6 (76, 82) ∥ 84
  Changes bar (74, 76) ∥ 85 Files (73, 74) ∥ 86 Terminal (76) ∥ 87
  Browser share (76) ∥ 88 turn undo (78) ∥ 89 approve card + note (77) ∥
  90 RPI gate bar (79) ∥ 91 runtimes gate (80) ∥ 92 palette (81).
  Wave 3: 93 transcript diff cards (88) ∥ 94 Changes actions + badge
  (74, 84) ∥ 95 Markdown TOC + Edit ∥ 96 plan-gate walk (90) ∥ 97
  docs. Every `confirm:` was run on the untouched tree at `d7d9406f2`+4
  research docs; the untouched-tree result is in parentheses.
- **Blocker.** No Rust confirm can run on this Mac today: every cargo
  build fails loading the built `sqlx-macros` proc-macro dylib
  ("mis-aligned LINKEDIT string pool", reproduced twice after deleting the
  artifact, `rustc 1.96.1`, Darwin 27.0.0). Tasks 77 and 79's Rust halves
  implement now and confirm when the toolchain is fixed (decision 6).
...
- 79. Plan-gate rule: `.agents/agents/orchestrator.md` §Task sizing — Normal and Unknown tiers end the turn after the Planner returns and implement only after the user's Accept (Tiny has no plan, no gate); `orchestratorRecipe(role, { planGate })` appends the same line; a per-project setting `workspace.planGate` (default on in Orchestrate) in `settings.ts` + `main.ts` allowlist; `session-controls.test.ts` "plan gate" cases.
  ...
  - confirm: `cd ui/desktop && pnpm vitest run src/workspace/session-controls.test.ts -t "plan gate" 2>&1 | grep -E 'Tests'` → contains `2 passed` (untouched: `11 skipped`); `grep -c "Accept" .agents/agents/orchestrator.md` → `≥ 1` (untouched: `0`)
...
- 83. "Add to chat" in six panes over task 76's door: Files (right-click → Add to chat; the file's path, or the selection when the Editor has one), Terminal (selection → a "Send to chat" pill; `TerminalSession.getSelection()`), Editor (selection → toolbar button), Changes (chunk header → Ask about this: the hunk's `+` side as the quote, `path:line-range` header — decision 4), Markdown (selection), Browser (Share ▸ Screenshot via `WebviewElement.capturePage` → `INSERT_INPUT_IMAGE`; Share ▸ Page stays).
  - status: todo · agent: — · worker: medium
  ...
  - UX test plan (walk `add to chat`): setup — scratch repo, session open, Terminal + Changes + Files open; steps — (1) Terminal: run `echo hello-t83`, select the output, click the pill → the input contains a fenced block whose header is `terminal` and body `hello-t83`; (2) Files: right-click `notes.md` → Add to chat → input gains `notes.md` as a `@`-style path line; (3) Changes: edit `notes.md`, open it, click the chunk's "Ask about this" → input gains a fenced block headed `notes.md:1-3` with the `+` lines; (4) Browser: load the sidecar `/health`, Share ▸ Screenshot → an image attachment tile appears in the input (count 1); (5) ⌘Enter is NOT pressed by any of these — the input still holds everything (`chat-input` value asserted); (6) Esc in the input keeps the text; states — each pane: ready (button enabled with a selection/target) · partial (no selection → button disabled with a title "Select text first") · error (capture failed → toast); phone: Files right-click = long-press (human), Terminal pill visible above the key bar; human-only: `capturePage` on a pane parked out of its slot, long-press
  - confirm: `cd ui/desktop && grep -c "getSelection" src/workspace/panes/terminal/terminal-session.ts` → `2` (untouched: `0`); `grep -c "capturePage" src/workspace/panes/browser/BrowserPane.tsx` → `1` (untouched: `0`); `just walk "add to chat"` → 1 passed (untouched: no spec)
...
- 87. Browser: Share ▸ Page · Screenshot · Console, dev-server default: `capturePage` typed on the `WebviewElement` interface; a `console-message` listener + a 200-line ring buffer in `browser-state.ts` (errors first); the Share button becomes a `DropdownMenu` (Page = today's text share; Screenshot = task 83's image door; Console = the buffer quoted as a `console` block); the dev-server default is per project in `project-storage.ts` (`goose.browser.home:<cwd>`), set from the address bar's "Set as home" (decision 11 — no repo file, nothing on `/config`).
  ...
  - confirm: `grep -c "browser-share-screenshot" ui/desktop/src/workspace/panes/browser/BrowserPane.tsx` → `1` (untouched: `0`); `just walk "browser pane"` → 1 passed
...
- 94. Changes: file-level actions, ask, jump, tab badge: row hover Stage file · Discard file (`/git/stage {paths}`, `/git/discard` — task 84's — scoped to one path via `git stash push -u -- <path>`); the hunk header's "Ask about this" (task 83) and "Open in Editor" at the hunk's first line (`openFile(path, line)`); `PaneChrome` gains a `badge` prop the Work column's tab bar renders (`WorkColumn.tsx`), filled from `gitStatus` as `3 · +40 −12` (files, then task 84's numstat) — decision 16.
  - status: todo · agent: — · worker: medium
...
- 96. The plan-gate walk and the live overrun count: `tests/e2e/plan-gate.spec.ts` per task 90's UX test plan (steps 1–7 + phone width); then ten live Hard runs on `claude-code` with the gate on, recorded in `docs/2026-09-18-plan-gate-runs-v1.md` (overrun count, wall clock, the model); > 1 overrun in ten → the spine gate (plan-mode research option (ii)) becomes a task — decision 13.
  ...
  - context: the walk needs a signed-in `claude`; the ten runs cost ten Hard turns (Claude Max)
...
6. **Rust toolchain** — the `sqlx-macros` dylib fails `dlopen` on Darwin 27 (`rustc 1.96.1`); recommend: try a hermit Rust refresh / `cargo clean` before anything else; until it builds, Rust tasks (77, 79's spine side) implement and land on a "confirm pending toolchain" note — the session reruns every Rust confirm the day it works.
7. **Files › Delete** — recommend: a fork trash dir (`<toplevel>/.goose-trash/`, gitignored) with session Undo, over OS Trash (needs a main-process IPC the phone cannot reach) or hard delete.
...
Approval → the list lands in `tasks.md` as tranche 7 and this section keeps
only the pointer; wave 1 starts at once (nine tasks, disjoint files — the
Rust ones subject to decision 6).
