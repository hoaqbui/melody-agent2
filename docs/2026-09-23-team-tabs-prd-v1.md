...
2. [Team health, ready] · P0
   ...
   - five tiles (task 273's list), each with a one-line "why" on hover like Telemetry's numbers:
     ...
     - **Stale notes** — `memories/**` pages past `stale_after` (the reminders rule, `melody-routines.py:183-197`)
...
- **Recommended: A**, with the `GOOSE_PATH_ROOT` fix in 271 so walks stop adding files. The 72 existing walk files are a one-time clean-up (open question 2); until then any that hold jobs show as extra repository rows named after their temp dir.
...
- [ ] given `GOOSE_PATH_ROOT` set, `/ledger/append` writes under `<root>/state/ledger/` and nothing under `XDG_STATE_HOME` — `cd ui/sidecar && pnpm vitest run ledger`
