...
- evaluation: every job gets an outcome from the ledger; judged jobs become each companion's test set; scores are DORA-style delivery, pass^k reliability, METR-style time horizon and tokens per clean job
...
1. **The four conflicts** in `docs/2026-09-23-melody-main-agent-research-v1.md` §Conflicts — proposed settlements:
   - **Companions are long-lived manager sessions**, one per repository, no cap (v3, the user's direction). "Fresh context per job" applies to the workers a companion delegates to. A companion's continuity is its folder, re-read whenever its seat compacts or restarts.
...
- **T4 — companion growth (B, then C).**
  - **B:** each companion's test set in `~/Melody/<name>/tests/` — a task needs a judged job (the user's verdict) and its expected result; a companion with < 3 judged jobs has no test set yet. A charter proposal reruns the set on the old and new charter: each task in a fresh worktree at the task's base commit, three runs each, graded by the task's grader (its tests, or the Reviewer's rubric); **pass^3 = all three pass**. The Scorecard: throughput, lead time, change failure rate, rework rate, tokens per clean job.
  - **C** (from ≥ 20 judged jobs per companion): job sizes, the time-horizon line, the autonomy ladder (suggest → worktree with review → merge after Reviewer PASS → larger jobs; two failures in a row drop a level).
...
- **T4 (B):** a fixture proposal that fails a task v2 passed is shown as "scores lower on <task>"; a companion with 2 judged jobs shows "not enough history"; each test run's worktree is removed after scoring.
