...
- 266. The job outcome fold: `ledger-outcome.ts`, pure, with fixtures
  - status: todo · agent: — · worker: high
  - card: as the user, I want each job to read failed, blocked, undone, reworked, corrected, landed or unknown by one fixed rule, so that "clean" means landed and nothing else (FURPS R · MoSCoW Must)
  - context: after 264; `outcomeOf(job, events, now)`, precedence failed > blocked > undone > reworked > corrected > landed > unknown — failed: status `failed` or verdict `wrong`; blocked: never clean; undone: an `undo` for the job's `turnId` no redo cancelled; reworked: only a `link`, file overlap only a `suggestions[]` entry; corrected: a `correction` or verdict `fixed`; landed: a `land` whose paths intersect `filesChanged` at or after the job; unknown: no land, no failure, `awaiting: true` under 7 days, never clean; what ships disagrees (`telemetry-now.ts:218-228` reads any done run as landed; `telemetry-roles.ts:143`)
  - confirm: `cd ui/desktop && pnpm vitest run ledger-outcome` → all fixtures pass (correction → corrected; linked fix → reworked; overlap alone does not; BLOCKED never landed; no land after 7 days → unknown; a replayed duplicate is one job; the precedence table) (today: no test file)
