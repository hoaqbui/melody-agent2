...
### The melody emblem (approved 2026-09-21, user: "we still don't have a melody emblem" → six sheets (notes · sideways lines · diagonals · fans · the tilted roll · the stair, longer) → "i like h5" → "is there an emblem too similar" → "show me h3 but on the same horizon line" → "let's do i4")

- 152. Three bars on one horizon: three equal bars of 10 units at 45°, 3.2 wide with round ends, centred at 6 · 12 · 18 on one line (`I4` on `sheet7.html`; H5 the stair was built first, then re-cut after the neighbour check — Ericsson stacks its three slanted capsules, so the stair was its cousin) — one shape drawn as three rounded rects on the 24-unit grid, filled with `currentColor`, no stroke, no gradient. It replaces the goose in place: the `Goose` component (`components/icons/Goose.tsx`, its name and three import sites unchanged — `src/components` is upstream's, the least diff), `glyph.svg` (the 22/44 px tray template, plus the update variant with its red dot), `icon.svg` (the mark at 60 % inside upstream's white rounded square) and every raster `prepare.sh` derives — `icon.png`, `icon@2x.png`, `icon-512.png`, `icon.ico`, `icon.icns`, the four `iconTemplate*.png`. `prepare.sh` renders through `qlmanage` (ImageMagick is not on this machine) and packs the `.ico` itself. Not in scope: the product name (`Goose`, `Goose.app`, the window title), the `Rain` hover lines in `GooseLogo`, the auto-updater feed.
  - status: done · agent: session · worker: low
    - 2026-09-21 (session): `Goose.tsx` draws the three rects; `glyph.svg`, `glyph-update.svg` (new — the red dot at 21,3 clear of the top bar) and `icon.svg` (the mark at `translate(376,376) scale(54)`, ~69 % of the square) rewritten; `prepare.sh` rewritten around `qlmanage` + `iconutil` + a python `.ico` packer and run — every raster regenerated (`file`: 22×22, 44×44, 1024, 2048, 512 RGBA; `.icns` "ic12" 66 KB, `.ico` 6 PNG entries); typecheck 0, eslint 0, prettier clean. The record is the rasters read back (`icon.icns` round-tripped through `iconutil -c iconset`, the tray PNGs on dark and light bars) — no walk shows the mark: in-chat it renders only on `ChatState.Idle`, the suspense loader is gone before the fixture attaches, and the dock icon is the packaged build's. Hand check: the dock and menu bar on the next `just make-ui`. Upstream's goose animations (`AnimatedIcons`, `FlyingBird` in `LoadingGoose`) still fly during Thinking/Streaming — a follow-up if wanted. `DESIGN.md` §Iconography amended (`grep -c "task 152"` → 1); `just walk "pane menu"` → 1 passed in 16.5 s (the app boots and paints with the new loader mark). Re-cut to I4 the same day: the three sources' rects replaced, `prepare.sh` rerun (`file`: 22×22, 44×44, 1024; `.icns` 60 KB), typecheck 0, eslint 0, the rasters read back again (`final-i4.html`); `just walk "pane menu"` → 1 passed (14.7 s).
  - card: as the user, see the stair on the dock, in the menu bar, on the loader and beside a recipe, so that the fork has a mark of its own
  - confirm: `grep -c "<rect" ui/desktop/src/components/icons/Goose.tsx` → `3` (untouched: `1`); `file ui/desktop/src/images/iconTemplate.png ui/desktop/src/images/iconTemplate@2x.png ui/desktop/src/images/icon.png` → `22 x 22`, `44 x 44`, `1024 x 1024`; `iconutil` exits 0 inside `prepare.sh`; `cd ui/desktop && pnpm run typecheck` → 0; `grep -c "task 152" DESIGN.md` → `1`; `just walk "pane menu"` → 1 passed (the loader renders `<Goose/>` on every launch)
