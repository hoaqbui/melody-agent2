...
- 154. A synchronous `delegate` from a `claude-code` orchestrator dies at ~60 s: the child keeps running but the tool call returns `<error>The operation timed out.</error>` and the orchestrator answers without the result. Reproduced headless twice on 2026-09-22 — the walk's own conditions (a roles-only scratch dir, `GOOSE_STATE_MACHINE=1`, seed 0 → agy: parent `20260922_10`, child `_11` created 21:01:05, "timed out" 63 s later) and a child told to `sleep 100` (parent `_13`, child `_14` at 21:03:55, the error quoted verbatim at 21:05:04). The desktop is not involved; `rpi strip` fails only because its researcher takes longer than a minute in a dir without the file it is asked about. Fix the cap at the bridge, then make the walk ask about a file the scratch dir holds.
  - status: doing · agent: session → haiku worker · worker: high
  ...
  - context:
    ...
    - 2026-09-22 (session, fix (b) landed): the cap is Claude Code's per-server `request_timeout_ms` (read from the `claude` 2.1.280 binary; `timeout` there is a different wall-clock watchdog), capped by Claude Code at 5 min; `claude_code.rs` `claude_mcp_config_json` now emits it for a StreamableHttp extension with a `timeout`, and the bridge sets `timeout: Some(300)` (`session_bridge.rs`). `McpServerHttp` (the ACP path, `claude-acp`) carries no timeout field, so Direct sessions on `claude-acp` keep the old cap. Confirm half 1 with the delegate forced synchronous: `SLEPT` after 2:23 (was "timed out" at 63 s). Half 2, `just walk "rpi strip"`: Research active → done → the Artifact pane shows Grok's brief (past `:67` for the first time), then fails at `:93` — the parent `20260923_1` in the walk profile holds only the user message, no assistant reply and no delegate tool row, 120 s after the child finished; the same prompt from the CLI replies. Open: why the desktop's claude-code orchestrator turn does not finish after a sync delegate returns (the next thing to read: goosed's log in the walk profile for session `20260923_1`)
    - later the same day, the lead: goosed's log for that walk (`$TMPDIR/goose-walks-profile/state/logs/cli/2026-09-22/20260922_172943.log`) shows "Session loaded" + "Restoring evicted session 20260923_1 (provider: Some(\"claude-code\"))" fourteen times between 00:30:36 and 00:31:39 — while the orchestrator's turn was live. A session load on the parent (from the desktop: likely the RPI strip's or Agents/Artifact pane's children read, rebuilt on load per PRODUCT.md §11) restores an "evicted" session, which probably replaces the running agent and strands the turn; the CLI does no loads and replies. Next: find who sends the loads (`goose::execution::manager` "Restoring evicted", and the desktop's `session/load` callers during a turn), and why a session with a live turn counts as evicted
    - the cause (session, sonnet worker, citations re-read): every ACP connection gets its own `AgentManager` (`acp/server.rs:2784` `connect_to` → `create_agent()`; `server.rs:969` `AgentManager::new`, not the shared instance), so a reconnect's `session/load` misses and `execution/manager.rs:212` builds a fresh Agent ("Restoring evicted"), orphaned from the one still running the turn. The desktop reconnects mid-turn: `acpConnection.ts:72-184` recovers the shared socket with backoff, and `ChatSessionsContainer.tsx:44-53` calls `restoreSession` on every open session after each recovery, with no active-turn guard (`chatSessionController.ts:145`). The CLI makes one agent for the process and never reconnects. Unknown: why the socket drops every 2–20 s while the parent waits on the delegate. Options (the user's call): (a) find and stop the drop — the sidecar's ACP proxy or a keepalive while a tool call holds the stream; (b) share one `AgentManager` across ACP connections like `active_runs` (`server_factory.rs:87-89`), a spine change that must keep `--roam`'s per-connection cwd isolation and re-attach to an in-flight stream; (c) the desktop skips `restoreSession` for a session with an active run — stops the duplicate agents, does not recover the lost stream. Recommended: (a) first, (c) with it; (b) as an upstream issue
