...
- **Task 21 revision (2026-09-15 21:05, spike run 2):** the fold delivered Goose's whole `subagent_system.md` template into the `claude-acp` child's user turn and Claude Code refused it as injection (`docs/2026-09-15-spine-bridge-spike-v1.md` §Run 2). Proposed task 26: fold only the role body (`system_instructions`, the `.agents/agents/*.md` content) framed as delegated instructions — e.g. `The orchestrator that delegated this task set your role as follows:` + body + `---` + task — keeping `override_system_prompt(subagent_prompt)` for providers that read it; rerun run 2 as the confirm. Owner: user — approve the revision or keep 21 as landed.
