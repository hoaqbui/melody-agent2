...
207 done 2026-09-24 (`f612236d9`, an unmerged run from 2026-09-23, reworked twice after Codex FAILs → PASS; merged `970b33fc8`: `melody_surface.rs` `MelodySurface` over the server's shared state; `start_session` refuses a missing mode/provider/model/extensions, a busy manager (claims the run via `ActiveRunRegistry`, released by a drop guard), a loaded manager (`ManagerLoaded`) and a non-`User` manager; extension state kept; `session/new` `_meta.role` only on `User` sessions; ARCHITECTURE.md "Melody surface"; confirm rerun on main: `melody_*` identity 6 · surface 9 · usage 1, `acp_server_test` 71, fmt and clippy clean. Two of its tests had written the test config over the real `~/.config/goose/config.yaml` — caught by the uncommitted guard in `acp_fixtures/mod.rs:205-213`, fixed in `69ab854ca` by running them under `run_test`). For 208: Codex's follow-up — serialize setup changes with activation and invalidate connection-cached agents (LRU-evicted but connection-cached, and an activation landing between `ManagerLoaded`'s check and the write, `melody_surface.rs:282`); 209's authz doesn't cover it.
...
- 208. The task Melody hands a manager runs at once with no window open, reaches any window that loads it, and a busy manager says so instead of waiting
  ...
  - context (2026-09-24, from 207's review): close the setup-vs-activation race — a setup change must be serialized with session activation, and connection-cached agents invalidated (`melody_surface.rs:282`; `acp/server.rs` `get_session_agent`'s fast path)
