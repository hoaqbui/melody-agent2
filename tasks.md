...
- 26. Fold only the role body into an ACP child's first user message: in `crates/goose/src/agents/subagent_handler.rs` change `first_user_message` to take the raw `system_instructions` (the `.agents/agents/*.md` content, `recipe.instructions`) instead of the rendered `subagent_prompt`, framed as `The orchestrator that delegated this task set your role as follows:\n\n<body>\n\n---\n\nYour task:\n\n<task>`; keep `override_system_prompt(subagent_prompt)` unchanged.
  - status: blocked · agent: — · worker: medium
  - blocked (2026-09-15 22:15): the code half landed (`c24a42a`, merged; role body only, framed as delegated instructions, 2 tests pass) but the live half failed 3 of 4 runs — Claude Code under `claude-agent-acp` 0.62.0 treats role text in a user turn as untrusted whatever the framing (child replies verbatim in the worker's report; dbs `scratchpad/sessions-run3{a,b,c,d}.db`). Unblocks: the user's pick on how a role reaches a Claude child — (A) put Claude worker seats on `claude-code` (system prompt via `--system-prompt-file`, 1/1 in run 1) and leave `claude-acp` for Direct sessions only; (B) find a system channel in `claude-agent-acp` (its `session/new` `_meta` or a config option — unverified); (C) accept the fold for Codex/Cursor ACP children (untested, task 9) and A for Claude — owner: user
