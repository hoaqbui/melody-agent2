...
- 278. The nightly tidy-up and its guard
  ...
  - context: `scripts/routines/tidy-up.yaml` — `working_dir: ~/Melody`, agy by default, `token_budget: 20000`, `min_seat_room: 20`; promote on two days, corrections overwrite, one `DREAMS.md` entry, commit `memory: tidy-up <date>` (`~/Melody/AGENTS.md` §Lifecycle); `scripts/melody-tidy-guard.py` (a script job at 03:30): every `journal/*.md` byte-identical, `MEMORY.md` loses ≤ 25 %, exactly one new `DREAMS.md` entry — the run works on a branch `tidy-up/<date>` in a temporary worktree of `~/Melody` (the user's checkout is never written); the guard checks `base..tidy-up/<date>` — the rules above plus an allowed-path list (`MEMORY.md`, `notes/`, `DREAMS.md`; no deletion elsewhere, no symlink at those paths) — and on pass merges `--ff-only` into `~/Melody`; a violation is not merged (branch and worktree removed, cause in the state file, and in `DREAMS.md` only by its own ff-only commit); ff refused because the user committed during the run → exit 2, branch kept. Redesigned 2026-09-23 after two Codex FAILs of revert-in-place (ownership, races with the user's edits, partial recovery); fixture `scripts/fixtures/notebook-tidy/`
  - confirm: `python3 scripts/melody-tidy-guard.py --fixture scripts/fixtures/notebook-tidy/ok` → `journal identical · dropped 12% · 1 dream`, exit 0; `…/journal-edited` → exit 1, not merged; `…/dropped-40` → exit 1; `…/deletes-soul` → exit 1; `…/user-commit-during-run` → exit 2, branch kept (today: the revert-in-place guard, twice failed by review)
