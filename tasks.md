...
Planned 2026-09-16 from the parity read (user: "plan it out all in tasks.md", then "implement and orchestrate with sub agents to parity"). The three gaps that stay felt daily after tranche 5: PR creation with CI status, a task board over parallel work, finish notifications. Order: 66 now (sidecar + Changes/Git, independent) ∥ 65; then 28 ∥ 67 ∥ 68 after 65 lands (67 and 68 read the delegations store); then 29 → 30.

- 66. Open a PR from the workspace and show its checks: sidecar routes in `ui/sidecar/src/git.ts` — `POST /git/pr/create {cwd, title, body, base?, draft?}` → `gh pr create --head <current branch> --title --body [--base] [--draft]` returning `{url, number}`; `POST /git/pr/status {cwd}` → `gh pr view --json number,url,state,isDraft,mergeable,statusCheckRollup` (+ `gh pr checks --json name,state,link` folded in) returning `{pr: {...} | null, checks: [{name, state: pending|pass|fail|skipped, link}]}`; `POST /git/push {cwd, setUpstream?}` → `git push [-u origin <branch>]`; every route under the task-48 `cwd` guard and the task-61 key; `gh` missing or not logged in → `503 {error: "gh not available: <stderr first line>"}` (the pane shows Install/Sign in per PRD §States). Desktop: `src/native/sidecar.ts` types; the Git pane gains a **PR** section under the commit box — no PR: "Push and open PR…" (push if ahead/no upstream, then a sheet: title from the last commit subject, body from the commits since the base, Draft checkbox) → the PR's number and URL; PR exists: state · checks list with lucide `CircleCheck`/`CircleX`/`CircleDashed` per check and a link that opens externally, refreshed on Refresh and every 60 s while the pane is visible; the Changes pane's worktree row (task 49) gains the same "Open PR…" beside Merge when the branch has an upstream.
  - status: doing · agent: subagent-t66 via claude-session-opus-2 (16:05, worktree) · worker: high
  - card: as the user, ship the reviewed change as a PR and watch its checks without leaving the window, so that the walk ends in GitHub, not in a terminal (Claude Code desktop's PR pane, Codex's and Emdash's PR creation)
  - context:
    - `gh` 2.100 is on this Mac and logged in (`gh auth status`, 2026-09-16); the sidecar shells to it with `execFile`, never a shell; the token never leaves `gh`'s keyring
    - `git.ts` already has `git()` with stdin (task 48); add a `gh()` sibling with the same `cwd` guard; `MAX_BODY_BYTES` bounds the body
    - the PR body is the user's to edit before Send — nothing posts without the sheet's Create button (AGENTS.md: publishing is explicit); `draft` default off
    - i18n `gitPane.pr*` keys in all 16 locales; DESIGN.md §Vocabulary "PR" row, §Iconography the three check icons; PRD step 7 amended (dated): "…Commit; Push and open PR"
    - tests: `git.test.ts` with a fake `gh` on PATH (a script under the test's tmpdir echoing canned JSON) — create, status with a failing check, 503 when the fake exits 4; the Electron walk uses the same fake `gh` (PATH prepended in the fixture) — never a real PR
  - confirm: `cd ui/sidecar && pnpm vitest run src/git.test.ts; echo exit=$?` → `exit=0` with ≥ 18 tests (untouched tree: 14); and `cd ui/desktop && pnpm run typecheck && pnpm exec playwright test -g "open pr"; echo exit=$?` → `exit=0` (untouched tree: no `open pr` spec — playwright exits 1; walk: commit on a branch, Push and open PR… → sheet prefilled → Create → number and URL shown → status shows one failing check from the fake)
