...
describe('gateState (plan gate)', () => {
  ...
  it('is false when gate is off or chat is not idle', () => {
    ...
    expect(gateState(views, false, true)).toBe(false);
  ...
  it('is false when implement has started or plan has not finished', () => {
    ...
    expect(gateState(phaseViews(implementRunning), true, true)).toBe(false);
    const planOnly = [row('planner', { status: 'running' })];
    expect(gateState(phaseViews(planOnly), true, true)).toBe(false);
  });
});